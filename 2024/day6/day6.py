...
def __find_new_direction(i: int, j: int, current_direction: Direction, puzzle_map: List[List[str]]):
...
def __has_escaped(i: int, j: int, direction: Direction, puzzle_map: List[List[str]]) -> bool:
    """ If the index went out of bound so did the guard """
    di = i + direction.direction[0]
    dj = j + direction.direction[1]
    return di < 0 or di >= len(puzzle_map) or dj < 0 or dj >= len(puzzle_map[0])


def __get_traversal_path(puzzle_map: List[List[str]]) -> Dict[Tuple[int, int], List[Direction]]:
    ...
    while not __has_escaped(i, j, direction, puzzle_map):
        ...
        i, j, direction, traversal_path, puzzle_map = __iterate_directions(i, j, direction, traversal_path, puzzle_map)

        # Already been on path with same direction
        future_i, future_j = i + direction.direction[0], j + direction.direction[1]

        if (future_i, future_j) in traversal_path and direction in traversal_path[(future_i, future_j)]:
...
def solve_6a(input_file: Path) -> int:
    ...
    traversal_path: Dict[Tuple[int, int], List[Direction]] = __get_traversal_path(puzzle_map)
